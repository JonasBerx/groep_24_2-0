...
public class RechthoekTest {
    ...
    @Test
    public void getOmhullende_returned_omhullende_met_juiste_parameters() throws DomainException {
        omhullende = new Omhullende(linkerBovenhoek, breedte, hoogte);
        assertEquals(omhullende.getLinkerBovenhoek(), rechthoek.getLinkerBovenhoek() );
        assertEquals(omhullende.getBreedte(), rechthoek.getBreedte());
        assertEquals(omhullende.getHoogte(), rechthoek.getHoogte());
    }
}